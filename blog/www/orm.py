...
# log()在此记录器上记录具有整数级别lvl的消息。其他参数解释为debug()。
def log(sql, args=()):
...
async def select(sql, args, size=None):
	...
	async with __pool.get() as conn:
		async with conn.cursor(aiomysql.DictCursor) as cur:
			# await 将调用一个子协程（也就是在一个协程中调用另一个协程）并直接获得子协程的返回结果
			await cur.execute(sql.replace('?', '%s'), args or ())
			# 如果传入size参数，就通过fetchmany()。。。
			# 否则就通过fetchall()...
			if size:
				# fetchmany()获得最多指定数量的记录
...
# execute()函数和select()函数不同的是，cursor对象不返回结果集
# 而是通过rowcount返回结果数
# 要执行增(insert)、删(delete)、改(update)，需要用execute()函数
...
#     name = StringField()
# 类级别上定义的属性用来描述User对象和表的映射关系
# 二实例属性必须通过__init__()方法取初始化
class Field(object):
...
# 通过metaclass，将具体的子类如User的映射信息读取出来
...
class Model(dict, metaclass = ModelMetaclass):
	...
	# 最后异步是完善ORM,对于查找，我们可以实现一下方法：
	#   · findAll() - 根据WHERE条件查找
	#   · findNumber() - 根据WHERE条件查找，但返回的是整数，设用与select count(*)类型的SQL
	@classmethod
	async def findAll(cls, where = None, args = None, **kw):
		...
		if limit is not None:
			...
			if isinstance(limit, int):
				...
				raise ValueError('Invalid lilmit value: {}'.format(str(limit)))
